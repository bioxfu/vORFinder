...
orfViewer <- function(x){
  ...
  orf_table <- data.frame(orf_label, orf_strand, orf_start, orf_end, orf_length, orf_sequence, stringsAsFactors = FALSE)
  ...
  write.table(orf_table, paste0("ORF/", x, ".table"), sep = "\t", row.names = FALSE, quote = FALSE)
  ...
  write.table(orf_model, paste0("ORF/", x, ".viewer"), sep = "\t", row.names = FALSE, quote = FALSE)
}
